...
export default function CategoryPage() {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setRecipes((prev) => prev.filter((r) => getId(r) !== String(id)));
      closeConfirm();
  ...
  const filteredRecipes = recipes.filter(
    (r) =>
      ...
      r.dessert_name?.toLowerCase().includes(search.trim().toLowerCase())
  );
      ...
      <div className="container my-4">
        <div className="row g-4 mt-2">
          {loading ? (
            ...
            <div className="text-center text-danger w-100 py-5">{error}</div>
